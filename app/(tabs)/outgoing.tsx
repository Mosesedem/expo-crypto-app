...
import { ArrowUpRight, Clock, CircleCheck as CheckCircle, Circle as XCircle, ExternalLink, Copy } from 'lucide-react-native';
...
export default function OutgoingScreen() {
  ...
  const loadOutgoingTransactions = async () => {
    try {
      const response = await apiService.getTransactions('outgoing');
      if (response.success && response.data) {
        setOutgoingTransactions(response.data);
      }
    } catch (error) {
      console.error('Failed to load outgoing transactions:', error);
    }
  };

  const onRefresh = async () => {
    setRefreshing(true);
    await loadOutgoingTransactions();
    setRefreshing(false);
  };
      ...
      {item.toAddress && (
        <View style={styles.addressContainer}>
          ...
          <TouchableOpacity
            ...
            onPress={() => copyToClipboard(item.toAddress!)}
          >
            <Text style={styles.addressText}>
              {item.toAddress.substring(0, 10)}...{item.toAddress.substring(item.toAddress.length - 10)}
            </Text>
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.headerTitle}>Outgoing Transactions</Text>
      ...
      {outgoingTransactions.length === 0 ? (
        <View style={styles.emptyState}>
          <ArrowUpRight size={64} color={COLORS.textSecondary} />
          <Text style={styles.emptyStateTitle}>No outgoing transactions</Text>
          <Text style={styles.emptyStateText}>
            Your withdrawals and sales will appear here
          </Text>
        </View>
      ) : (
        <FlatList
